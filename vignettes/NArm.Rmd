...
if (requireNamespace("RSQLite")) {
  fnam <- tempfile(pattern = "replyr_doc_narm", tmpdir = tempdir(), fileext = "sqlite3")
...
d %>% group_by(rowNum) %>% summarize_all(c("is.na")) %>%
  mutate(nNAinRow=rowSums(.)-rowNum)

dRemote %>% group_by(rowNum) %>% summarize_all(c("is.na")) %>% 
  mutate(nNAinRow=rowSums(.)-rowNum)
```
