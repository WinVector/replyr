...
```{r render1}
pngFileName <- 'jPlanDependencySorting.png'
diagramPNG <- NULL
# requireNamespace checks just for strict warning hygiene in vignette
if(!is.null(sorted)) {
  have <- c(
  ...
  if(all(have)) {
    diagramPNG <- sorted$columnJoinPlan %>%
      makeJoinDiagramSpec() %>%
      renderJoinDiagram() %>%
      convertDiagrameToPNG(pngFileName = pngFileName)
  }
}
```
...
And this is how we use tools to do the heavy lifting in building a left join plan:
  ...
  * `replyr::makeJoinDiagramSpec() %>% replyr::renderJoinDiagram() %>% convertDiagrameToPNG()` can then produce and capture a detailed diagram of the final join plan.
