...
This system has largely been replaced by the [`rquery` equivalent](https://github.com/WinVector/rquery/blob/master/extras/DependencySorting.md).
...
```{r sort, eval=execute_vignette, error=TRUE}
sorted <- NULL
...
```{r sortplt, eval=FALSE, error=TRUE}
if(!is.null(sorted)) {
...
```{r diagram, eval=FALSE, error=TRUE}
sorted$columnJoinPlan %>%
        makeJoinDiagramSpec(.) %>%
        DiagrammeR::grViz(.)
```

![](DependencySorting_d.png)
...
And this is how we use tools to do the heavy lifting in building a left join plan:
  ...
  * `replyr::makeJoinDiagramSpec() %>% DiagrammeR::grViz(.)` can then produce and capture a detailed diagram of the final join plan.
