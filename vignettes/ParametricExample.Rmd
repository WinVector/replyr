...
Consider the problem of "parametric programming."  That i:s simply writing correct
...
is deliberately straightforward, but programming over `dplyr` can  become a challenge.
...
unevaluated.  The code looks like the following (but does  not work for `dplyr version '0.5.0' or '0.5.0.9000' latest Github and dev as of 1-5-2017`):
 
```{r}
if  (requireNamespace("lazyeval")) {
  print(d %>% mutate_(RCOL = lazyeval::interp(~ is.na(cname))) %>%
          rename_(.dots = stats::setNames('RCOL', rname)))
}
```

Currently `mutate_` does not take "two-sided formulas" so we need to control
names outside of the formula.
In this case we used the explicit `dplyr::rename_` because attempting to name the
assignment in-line does not seem to be supported (or if it is supported, it uses 
a different notation or convention than the one we have just seen, (but does  not work for `dplyr version '0.5.0' or '0.5.0.9000' latest Github and dev as of 1-5-2017`):

```{r}
# the following does not correctly name the result column
if  (requireNamespace("lazyeval")) {
  print(d %>% mutate_(.dots = stats::setNames(lazyeval::interp( ~ is.na(cname)),
                                              rname)))
}
```
...
(what the linked NSE reference called "mixing constants and variable").  We have
a hard time getting control of incoming and outgoing variables.
...
Our advice is to give [`replyr::let`](https://github.com/WinVector/replyr) a try.
`replyr::let` takes a name mapping list (called "`alias`") and a code-block (called "`expr`").
...
tasks the ratio of `replyr::let` boilerplate to actual useful work goes down quickly.

We also have a varation for piping into (though to save such pipes for later you use 
`replyr::let`, not `replyr::letp`):
...
The alias map is deliberately only allowed to be a string to string map (no environments, `as.name`, `formula`, expressions, or values) so `replyr::let` *itself* is easy to use in automation or program over. 
I'll repeat that for emphasis: externally `replyr::let` is completely controllable through standard
 ...
 * [The `replyr::let` help documentation](http://www.win-vector.com/blog/2016/12/helplet-packagereplyr/)
