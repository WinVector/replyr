...
(source: [dplyr Non-standard evaluation](https://cran.r-project.org/web/packages/dplyr/vignettes/nse.html), for additional theory and upcoming official solutions please see [here](https://rpubs.com/hadley/lazyeval)).
...
unevaluated.  The code looks like the following:
...
a different notation or convention than the one we have just seen):
...
(what the linked NSE reference called "mixing constants and variable").  We
can only conveniently get about halfway to the solution (output parameterized,
input hard-coded as "x").
